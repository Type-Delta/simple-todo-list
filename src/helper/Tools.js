      ...
      */
      ...
      createTracker = function(func, name, tracer) {
         ...
         const getString = (obj) => {
            ...
            if(obj instanceof Map){
               const values = [...obj.entries()]
               return 'Map: ' + JSON.stringify(values, null, values.length > 5? 3: null);
            }

            if(obj instanceof Set){
               const values = [...obj.values()];
               return 'Set: ' + JSON.stringify(values, null, values.length > 5? 3: null);
            }

            try{
               if(typeof obj === 'object'){
                  let itemCount = obj instanceof Array? obj.length: Object.keys(obj).length;
                  return JSON.stringify(obj, Tools.JSONReplacer, itemCount > 5? 3: null);
               }
            }catch{}
         ...
         return function() {
            ...
            for(let i = 0; i < arguments.length; i++) {
               ...
               const stringifiedArg = getString(arguments[i]);
            ...
            else logText += '): ' + limit(getString(returnValue), 70);
            tracer.log += logText + '\n';
